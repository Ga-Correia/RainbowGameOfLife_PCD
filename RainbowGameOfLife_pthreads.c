...
void *geracoes(void *arg) {
  Dados_Thread *dados = (Dados_Thread *)arg;
  int inicio = tamanho / qtd_thread * (dados->id);
  int fim = inicio + tamanho / qtd_thread;
  int ite, soma;
  float media;
  float **swap;
...
int main(int argc, char **argv) {
  ...
  for (i = 0; i < qtd_thread; i++) {
    ...
    *id = i;
    ...
    pthread_create(&thr[i], NULL, geracoes, (void *)dados);
  }
